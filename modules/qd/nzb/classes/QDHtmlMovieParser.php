<?php
	...
	class QDHtmlMovieParser{
		...
		function getParserClass($link){
			$class = null;
			$arrParsers = array(
					'QDHtmlMovieParserAllocine'			=>'!allocine\.fr!',
					//'QDHtmlMovieParserCinemotions'		=>'!cinemotions\.com!',
					//'QDHtmlMovieParserCinemajeuxactu'	=>'!cinema\.jeuxactu\.com!',
			);
		...
		function getDescFromLink($link){
			$desc=array();
			$className = QDHtmlMovieParser::getParserClass($link);
			if($className){
				$t = new $className();
				try{
					set_time_limit(5);
