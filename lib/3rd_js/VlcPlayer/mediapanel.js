...
Ext.ux.MediaPanel = Ext.extend(Ext.Panel , Ext.apply({
    ...
    ,initComponent : function(){
      ...
      //    console.log("MediaPanel initComponent "+this.mediaCfg.unsupportedText);
...
Ext.ux.MediaPanel.Flash = Ext.extend(Ext.ux.MediaPanel, {
    ...
    /*
    * Asserts the desired version against the installed Vlc Plugin version.
    ...
    */
    ...
    /*
    * Vlc Plugin version detection function
    ...
    * false if Vlc is not installed or detection failed.
    */
...
Ext.ux.MediaPanel.Vlc = Ext.extend(Ext.ux.MediaPanel, {
    autoStart      : false,
    requiredVersion: false,
    playerVersion  : null,
    ctype          : "Ext.ux.MediaPanel.Vlc",
    VLCObject      : null,
    "_VLC" : Ext.apply({
          tag      : 'object'
        ,cls      : 'x-vlc-plugin'
         ...
         ,params   : {}
    },Ext.isIE?{classid :"clsid:9BE31822-FDAD-461B-AD51-BE1D1C159921"}:false),

    // private
    initComponent : function(){

        var mc = Ext.apply({url:"test"}, this.mediaCfg||{});
       
        if(1){

            this.requiredVersion = mc.requiredVersion||false;

            var hasVlc  = !!(this.playerVersion = this.detectVersion());
            var hasRequired = hasVlc && (this.requiredVersion?this.assertVersion(this.requiredVersion):true);
        ...
        Ext.ux.MediaPanel.Vlc.superclass.initComponent.call(this);
    }
    ,onRender  : function(ct, position){
       Ext.ux.MediaPanel.Vlc.superclass.onRender.call(this, ct, position);
       this._setVLCObject.defer(20,this);
     }
     //See superclass for details
    ,renderMedia : function(mediaCfg){
       this.VLCObject = null;
       Ext.ux.MediaPanel.Vlc.superclass.renderMedia.call(this, mediaCfg);
       this._setVLCObject.defer(20,this);
     }

     //Private
     ,_setVLCObject: function(){
       if(this.VLCObject = Ext.get(this.body.child('object.x-vlc-plugin'))){
           this.VLCObject.setVisibilityMode(Ext.Element[this.hideMode.toUpperCase()] || 1 ).originalDisplay = (this.hideMode != 'display'?'visible':'block');
       }
  }
    ...
    * Modifed version of the detection strategy of SWFObject library : http://blog.deconcept.com/swfobject/
    * returns a {major,minor,rev} version object or
    * false if Vlc plugin is not installed or detection failed.
    */
    detectVersion : function(){
        var version=false;
        var formatVersion = function(version){
        ...
        if(Ext.isIE){
            var sfo= null;
            try{
                sfo = new ActiveXObject("VideoLAN.VLCPlugin.2");
            }catch(e){
                try {
                    sfo = new ActiveXObject("VideoLAN.VLCPlugin");
                    version = [0,8,6];
                } catch(e) {}
            }
            if (sfo) {
                version = sfo.VersionInfo.split(", ")[0].split(",");
            }
         }else if(navigator.plugins && navigator.mimeTypes.length){
            var sfo = navigator.plugins["VLC Multimedia Plug-in"];
            if (!sfo) sfo = navigator.plugins["VLC Multimedia Plug in"];
            if (sfo && sfo.description) {
                sfo = sfo.description.split(", ")[0].split("Version ")[1]
                version = sfo.split(".");
            }    
        }
        return formatVersion(version);
    ...
    beforeDestroy : function(){
        ...
        Ext.ux.MediaPanel.Vlc.superclass.beforeDestroy.call(this);
    },
    
    getVLCObject : function() {
        return this.rendered?this.VLCObject.dom||null:null;
    },
   
     // private
     onShow : function(){
    	 if(this.VLCObject){
           this.VLCObject.setVisible(true);
         }
         Ext.ux.MediaPanel.Vlc.superclass.onShow.call(this);
     }

     // private
    ,onHide : function(){
    	if(this.VLCObject){
         this.VLCObject.setVisible(false);
        }
        Ext.ux.MediaPanel.Vlc.superclass.onHide.call(this);
    }

});


Ext.reg('vlcpanel', Ext.ux.MediaPanel.Vlc);