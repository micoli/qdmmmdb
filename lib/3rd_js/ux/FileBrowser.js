Ext.define('Ext.ux.FileBrowser', {
	...
	viewPreview			: true,
	tbarConfig			: null,
	folderTreeConfig	: null,
	fileGridConfig		: null,
	withSelection		: false,
	withEditing			: false,
	withFileRenaming	: false,
	...
	initComponent	: function(){
		...
		Ext.define('fileBrowserFiles', {
			extend: 'Ext.data.Model',
			fields: ['selected','folder', 'type', 'filename', 'ext', 'cdate', 'mtime', 'size', 'sizef', 'sizefs', 'root', 'owner', 'group', 'perms', 'preview', 'folderfilename','permt','permo','permg','perma']
		});
		...
		if(that.tbarConfig){
			tbar = Ext.ux.array.insertArrayAt(tbar,2,that.tbarConfig);
		}
		Ext.apply(this,{
			items	:[Ext.apply({
				xtype			: 'treepanel',
				...
				items			: [Ext.apply({
					...
					columns			: [{
						...
						header: "Name"		,width: 200,	dataIndex: 'filename'	, flex	:1	,sortable: true,editor:(that.withEditing && that.withFileRenaming)?{
							allowBlank	: false
						}:null
					},{
		...
		if(that.withEditing){
			this.items[1].items[0].plugins = [{ptype: 'cellediting', clicksToEdit: 1}];
		}
		if (that.withSelection){
			this.items[1].items[0].columns.unshift({
				header: "&nbsp", width: 25,flex : 0, dataIndex: 'selected', sortable: true, xtype: 'checkcolumn',listeners : {
					checkchange : function(event,idx,value,checkbox){
						var record = that.gridstore.getAt(idx);
						record.commit();
						that.fireEvent('selectionchanged',that,that.gridstore,idx,record,value);
					}
				}
			});
		}
