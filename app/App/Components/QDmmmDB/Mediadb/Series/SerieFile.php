...
class SerieFile{
	...
	public function __construct($sFullFilename)
	{
		...
		foreach ($GLOBALS['conf']['qdmediadb']['arrRegex'] as $k => $rgx) {
	...
	{
		return in_array(strtolower($this->extension), $GLOBALS['conf']['qdmediadb']['allowedExt']);
	}

	public function isSubtitleFile()
	{
		return in_array(strtolower($this->extension), $GLOBALS['conf']['qdmediadb']['subtitlesExt']);
	}
