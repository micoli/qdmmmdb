...
class scrapertheMovieDBApiV2 extends MovieParser{
	...
	function getList($movieName){
		...
		foreach($arrResult as $k=>$v){
			if(is_array($v)){
				$searchresults[]=array(
					'id'		=> $v['id'],
					'title'		=> Tools::array_key_exists_assign_default('name', $v, '').($v['original_name']?' ('.$v['original_name'].')':''),
	...
	function getDetail($id){
		...
		$arr = array_pop(Tools::object2array(json_decode($this->QDNet->getCacheURL($url,'themoviedbapi',$this->cacheminutes,$this->cache))));
		//db($arr);die();
